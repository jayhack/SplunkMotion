...
class Autosynch_Interface:
	...
	def record (self):
		...
		system_command = "./x64-release/SplunkMotion record " + self.selected_recording.full_filepath
	...
	def watch (self):
		...
		if not selection in jvids_map.keys ():
			...
			### Step 2: build the correct command and use it ###
			system_command = "./x64-release/SplunkMotion " + jvids_map[selection] + " " + self.selected_recording.full_filepath
			os.system (system_command)
